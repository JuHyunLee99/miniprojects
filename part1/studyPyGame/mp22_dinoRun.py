...
# 공룡이미지 로드
RUNNING = [pygame.image.load('./studyPyGame/Assets/Dino/DinoRun1.png'),
...
Death = pygame.image.load(f'{ASSETS}DINO/DinoDead.PNG')
# 구름이지미
...
# 선인장 이미지 로드 / 애니메이션이 아니라 선인장 종류가 세개씩
...
class Dino: # 공룡 클래스
    ...
    def update(self, userInput) -> None:
        ...
        if userInput[pygame.K_UP] and not self.dino_jump:   # 점프
            ...
            self.dino_jump = True
            self.dino_rect.y = self.Y_POS # 이게없으면 공룡이 하늘로 날라감
...
class Cloud: # 구름 클래스
    def __init__(self) -> None:
        self.x = SCREEN_WIDTH + random.randint(300, 500)    # random 다음 구름
    ...
    def update(self) -> None:
        ...
        if self.x < -self.width: # 화면 밖으로 벗어나면
            self.x = SCREEN_WIDTH + random.randint(1300,2000)   # 다음구름
            self.y = random.randint(50, 100)
...
class Bird(Obstacle):   # 장애인 상속 클래스
    ...
    def draw(self, SCREEN) -> None: # draw 정의
        if self.index >= 9:
            self.index = 0
...
class LargeCactus(Obstacle):    
    def __init__(self, image) -> None:
        self.type = random.randint(0, 2)    # 큰 선인장 세개중 하나 고름
        super().__init__(image, self.type)
        self.rect.y = 300

class SmallCactus(Obstacle):    
    def __init__(self, image) -> None:
        self.type = random.randint(0, 2)    # 작은 선인장 세개중 하나 고름
        super().__init__(image, self.type)
        self.rect.y = 325
def main():
    global game_speed, x_pos_bg, y_pos_bg, points, obstacles, font, death_count
    ...
    # 함수 내 함수(배경그리기)
    def backgound():    # 땅바닥 update, drw 동시에 해주는 함수
...
def menu(death_count):  # 메인 함수
    ...
    while run:
        ...
        if death_count == 0: # 최초
            ...
            SCREEN.blit(Death, (SCREEN_WIDTH // 2 - 20, SCREEN_HEIGHT // 2 - 140))
