# 대량 메일 전송
from openpyxl import load_workbook  # pip install openpyxl  # excel 핸들링을 위한 모듈
import smtplib
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart

wbook = load_workbook('./studyPython/SpamMailList.xlsx', data_only=True)    # SpamMailList.xlsx 엑셀파일 로드
wsheet = wbook.active # sheet1 선택

for i in range(1, wsheet.max_row+1):    # sheet1 내용 가져오기
    recv_mail = wsheet.cell(i, 1).value
    print(recv_mail)
    try:
        # 실제 메일전송 로직
        send_mail = 'dlwngus0829@naver.com'
        send_pass = ''
